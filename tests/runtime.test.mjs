...
test("transfer delegates the current Claude session directly to native import", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      { type: "user", cwd: repo, message: { role: "user", content: "/codex-stable:transfer" } }
  ...
  assert.deepEqual(
    ...
    ["Initial request", "Initial answer", "/codex-stable:transfer"]
  );
});
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/codex-stable:adversarial-review focus on auth/i);
});
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  assert.match(result.stdout, /`\/codex-stable:status review-live`<br>`\/codex-stable:cancel review-live`/);
...
test("status --wait uses the timeout from the environment", () => {
  ...
  fs.writeFileSync(logFile, "[2026-03-18T15:30:00.000Z] Starting Codex Task.\n", "utf8");
  fs.writeFileSync(path.join(jobsDir, "task-live.json"), JSON.stringify(job, null, 2), "utf8");
  fs.writeFileSync(
    path.join(stateDir, "state.json"),
  ...
  const result = run("node", [SCRIPT, "status", "task-live", "--wait", "--json"], {
    cwd: workspace,
    env: { ...process.env, CODEX_COMPANION_STATUS_WAIT_TIMEOUT_MS: "25" }
  });
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/codex-stable:status/i);
  assert.match(blocked.stderr, /\/codex-stable:cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/codex-stable:setup/i);
});
