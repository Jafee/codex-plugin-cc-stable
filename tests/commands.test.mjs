...
test("adversarial review command uses AskUserQuestion and background Bash while staying review-only", () => {
  ...
  assert.match(source, /When in doubt, run the review/i);
  assert.match(source, /\(Recommended\)/);
  assert.match(source, /uses the same review target selection as `\/codex-stable:review`/i);
...
test("rescue command absorbs continue semantics", () => {
  ...
  // Regression for #234: `Skill(codex-stable:rescue)` from the main agent recursed
  // because rescue.md named the routing with ambiguous prose ("Route this
  // request to the `codex-stable:codex-rescue` subagent") while running under
  ...
  assert.match(rescue, /subagent_type: "codex-stable:codex-rescue"/);
  assert.match(rescue, /do not call `Skill\(codex-stable:codex-rescue\)`/i);
  ...
  assert.match(rescue, /run the `codex-stable:codex-rescue` subagent in the background/i);
  ...
  assert.match(readme, /`codex-stable:codex-rescue` subagent/i);
  assert.match(readme, /if you do not pass `--model` or `--effort`, Codex chooses its own defaults/i);
  assert.match(readme, /--model gpt-5\.4-mini --effort medium/i);
  assert.match(readme, /`spark`, the plugin maps that to `gpt-5\.3-codex-spark`/i);
  assert.match(readme, /continue a previous Codex task/i);
  assert.match(readme, /### `\/codex-stable:setup`/);
  assert.match(readme, /### `\/codex-stable:review`/);
  assert.match(readme, /### `\/codex-stable:adversarial-review`/);
  assert.match(readme, /uses the same review target selection as `\/codex-stable:review`/i);
  assert.match(readme, /--base main challenge whether this was the right caching and retry design/);
  assert.match(readme, /### `\/codex-stable:rescue`/);
  assert.match(readme, /### `\/codex-stable:transfer`/);
  assert.match(readme, /### `\/codex-stable:status`/);
  assert.match(readme, /### `\/codex-stable:result`/);
  assert.match(readme, /### `\/codex-stable:cancel`/);
});
...
test("setup command can offer Codex install and still points users to codex login", () => {
  ...
  assert.match(readme, /\/codex-stable:setup --enable-review-gate/);
  assert.match(readme, /\/codex-stable:setup --disable-review-gate/);
});
