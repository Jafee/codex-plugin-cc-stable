...
Use this skill only inside the `codex-stable:codex-rescue` subagent.
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `codex-stable:codex-rescue`.
...
Safety rules:
- Default to write-capable Codex work in `codex-stable:codex-rescue` unless the user explicitly asks for read-only behavior.
