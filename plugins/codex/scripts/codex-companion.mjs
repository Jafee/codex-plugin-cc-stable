...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!config.stopReviewGate) {
    nextSteps.push("Optional: run `/codex-stable:setup --enable-review-gate` to require a fresh review before stop.");
  }
...
function ensureCodexAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Codex CLI is not installed or is missing required runtime support. Install it with `npm install -g @openai/codex`, then rerun `/codex-stable:setup`.");
  }
}
...
function validateNativeReviewRequest(target, focusText) {
  if (focusText.trim()) {
    throw new Error(
      `\`/codex-stable:review\` now maps directly to the built-in reviewer and does not support custom focus text. Retry with \`/codex-stable:adversarial-review ${focusText.trim()}\` for focused review instructions.`
    );
  }

  const nativeTarget = buildNativeReviewTarget(target);
  if (!nativeTarget) {
    throw new Error("This `/codex-stable:review` target is not supported by the built-in reviewer. Retry with `/codex-stable:adversarial-review` for custom targeting.");
  }
...
async function resolveLatestTrackedTaskThread(cwd, options = {}) {
  ...
  if (activeTask) {
    throw new Error(`Task ${activeTask.id} is still running. Use /codex-stable:status before continuing it.`);
  }
...
function renderQueuedTaskLaunch(payload) {
  return `${payload.title} started in the background as ${payload.jobId}. Check /codex-stable:status ${payload.jobId} for progress.\n`;
}
...
async function handleTransfer(argv) {
  ...
  // Transfer starts Codex work in this directory, so fail fast on a typo or
  // vanished worktree (fork deviation from upstream's lenient resolve).
  const cwd = resolveCommandCwd(options, { requireExists: true });
