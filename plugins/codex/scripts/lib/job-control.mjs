...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run /codex-stable:status to list known jobs.`);
}
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /codex-stable:status and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run /codex-stable:status to inspect active jobs.`);
  }
